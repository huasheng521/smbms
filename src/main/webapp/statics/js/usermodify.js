		...
		success:function(data){//data：返回数据（json对象）
			if(data != null){
				...
				for(var i = 0; i < data.roleList.length; i++){
					...
					if(rid != null && rid != undefined && data.roleList[i].id == rid ){
						options += "<option selected=\"selected\" value=\""+data.roleList[i].id+"\" >"+data.roleList[i].roleName+"</option>";
					}else{
						options += "<option value=\""+data.roleList[i].id+"\" >"+data.roleList[i].roleName+"</option>";
					}
